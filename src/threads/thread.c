               ...
               thread_func *function, void *aux) 
{
  ...
  list_push_back (&thread_current()->children_list, &t->thread_child->child_elem);
  //相当于把新线程放进子线程列表中
...
thread_exit (void) 
{
  ...
  thread_current ()->thread_child->store_exit = thread_current()->exit_code;
  //子线程退出，把资源还给父线程
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->exit_code = -1;   //assume the exit_code -1 unless the thread exit properly 
