...
process_execute (const char *file_name) 
{
  ...
  char *command = strtok_r(fn_copy0," ",&save_ptr);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (command, PRI_DEFAULT, start_process, fn_copy1);
  palloc_free_page(fn_copy0);
  if (tid == TID_ERROR){
  ...
  if (!thread_current()->success) return TID_ERROR;//can't create new process thread,return error  return tid;
  return tid;
}
...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp); //load a program
  ...
  if (pd != NULL) 
    {
      ...
      if(cur->supt != NULL){
          // lock_acquire(&cur->supt->supt_lock);
          supt_destroy(&cur->supt->page_map);
          // lock_acquire(&cur->supt->supt_lock);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      //     return false; 
      //   }
      if(!lazy_load_frame(file,ofs,upage,page_read_bytes,page_zero_bytes,writable))
          return false;
  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
          *esp = PHYS_BASE;
          ...
          uint32_t* arg_pointer_saver[argc];


          for(int i=argc-1;i>=0;i--){
              *esp = *esp - sizeof(char) * (strlen(argv[i])+1);
              memcpy(*esp,argv[i],sizeof(char)*strlen(argv[i])+1);
              arg_pointer_saver[i] = (uint32_t *)*esp;
          } //push args and save their address
          
          *esp = *esp-4;
          (*(int *)*esp) = 0;//need a null sentinel for the 2 demesion array

          *esp = *esp - 4;
          for(int i = argc-1;i>=0;i--){
              (*(uint32_t**)(*esp)) = arg_pointer_saver[i];
              *esp = *esp-4;
          }
          
          (*(uintptr_t**)(*esp)) = *esp +4; //save the start address of pointers
